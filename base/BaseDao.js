...
class BaseDao {
    ...
    commit (connection) {
        return new Promise((resolve, reject) => {
            connection.commit(err => err ? reject(err) : resolve(conn));
        });
    }

    rollback (connection) {
        return new Promise((resolve, reject) => {
            connection.rollback(err => err ? reject(err) : resolve(conn));
        });
    }

    selectBySql (sql, condition, connection) {
        return this.query(`${sql} ${condition ? condition.toSql() : ''}`, condition && condtion.toValue(), connection);
    }
    ...
    _execute (callback, conn) {
        return new Promise((resolve, reject) =>
            conn ? callback(conn, resolve, reject) : this.getConnection().then(conn => callback(conn, resolve, reject)));
    }
